...
import (
	...
	apisCluster "github.com/valkiriaaquatica/provider-stripe/apis/cluster"
	apisNamespaced "github.com/valkiriaaquatica/provider-stripe/apis/namespaced"
	"github.com/valkiriaaquatica/provider-stripe/config"
	"github.com/valkiriaaquatica/provider-stripe/internal/clients"
	controllerCluster "github.com/valkiriaaquatica/provider-stripe/internal/controller/cluster"
	controllerNamespaced "github.com/valkiriaaquatica/provider-stripe/internal/controller/namespaced"
	"github.com/valkiriaaquatica/provider-stripe/internal/features"
	"github.com/valkiriaaquatica/provider-stripe/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for STRIPE").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-stripe"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-stripe",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped STRIPE APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced STRIPE APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped STRIPE controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced STRIPE controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped STRIPE controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced STRIPE controllers")
	}
